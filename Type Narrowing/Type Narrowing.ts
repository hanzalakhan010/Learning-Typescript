 ...
 */
function getPizzaDetail(id?: Number, identifier?: String): Pizza | undefined {
    if (id) {
        const pizza: Pizza | undefined = menu.find((pizza => pizza.id === id))
        return pizza
    }
    else if (identifier) {
        const pizza: Pizza | undefined = menu.find((pizza => pizza.name === identifier))
        return pizza
    }
    else {
        return undefined
